...
public final class MixinCompatibilityGameTests
{
    ...
    public static void sameBlockTypeStateChangeDoesNotPublishBlockChangedEvent(GameTestHelper helper)
    {
        ...
        try
        {
            ...
            helper.assertTrue(observer.count == 0, "Same-block property change published a block-change event");
            helper.setBlock(changedPos, Blocks.STONE);
            helper.assertTrue(observer.count == 1, "Different-block change did not publish exactly one event");
        }
        finally
        {
            Constants.EVENT_BUS.unregister(observer);
    ...
    public static void displayAccessors(GameTestHelper helper)
    {
        ...
        helper.assertTrue(DisplayProbes.transformationInterpolationDuration(display) == 7,
                "Display interpolation duration invoker did not update synced data");
        helper.assertTrue(DisplayProbes.transformationInterpolationDelay(display) == 3,
                "Display interpolation delay invoker did not update synced data");
