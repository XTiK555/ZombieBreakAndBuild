...
public class BuildDisappearBlockVisual
{
    ...
    public void onServerStopping(MainCommon.OnServerStoppingEvent event)
    {
        for (ServerLevel level : event.server().getAllLevels())
        {
            level.getAllEntities().forEach(entity ->
            {
                ...
                {
                    if (blockDisplay.getTags().contains(DISPLAY_TAG))
                    {
                                    ...
                                    double x, double y, double z, float scale, int interpolationDuration)
    {
        ...
        displayData.put(Display.TAG_TRANSFORMATION,
                Transformation.EXTENDED_CODEC.encodeStart(NbtOps.INSTANCE, transformation).result().orElseThrow());
        displayData.putInt(Display.TAG_START_INTERPOLATION, 0);
        displayData.putInt(Display.TAG_INTERPOLATION_DURATION, interpolationDuration);

        blockDisplay.load(displayData);
