...
public final class FallingBlockStorageTracker
{
    ...
    private final SavedData.Factory<TrackerSavedData> savedDataFactory;
    ...
    {
        Codec<List<PersistedEntry>> codec = PersistedEntry.CODEC.listOf()
                .optionalFieldOf("entries", List.of())
                .codec();

        savedDataFactory = new SavedData.Factory<>(
                () -> new TrackerSavedData(List.of(), codec),
                (tag, registries) -> new TrackerSavedData(codec.parse(
                        registries.createSerializationContext(NbtOps.INSTANCE), tag).getOrThrow(), codec),
                null
        );
    ...
    private TrackerSavedData savedData(ServerLevel level)
    {
        return savedDataByLevel.computeIfAbsent(
                level,
                ignored -> level.getDataStorage().computeIfAbsent(savedDataFactory, savedDataName)
        );
    }
    ...
    static final class TrackerSavedData extends SavedData
    {
        ...
        {
            this(entries, PersistedEntry.CODEC.listOf()
                    .optionalFieldOf("entries", List.of())
                    .codec());
        }
