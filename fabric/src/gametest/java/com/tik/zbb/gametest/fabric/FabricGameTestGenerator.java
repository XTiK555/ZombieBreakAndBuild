...
public final class FabricGameTestGenerator
{
    ...
    private static void generate(Path output, Path resources) throws Exception
    {
        ...
        for (PortabilityGameTestScenarios.Suite suite : PortabilityGameTestScenarios.SUITES)
        {
            ...
            {
                source.append("        @GameTest(structure = \"").append(PortabilityGameTestScenarios.EMPTY_STRUCTURE_ID)
                        .append("\", environment = \"").append(suite.environmentId(scenario))
                        .append("\", maxTicks = ").append(scenario.maxTicks()).append(")\n")
                        .append("        public void ").append(toLowerCamel(scenario.id())).append("(GameTestHelper helper)\n")
